                            ...
                            unsigned int ms)
{
    ...
    mutex.lock();
    m_van->show();
    QPropertyAnimation *animation=new QPropertyAnimation(m_van, "pos");
...
{
    QParallelAnimationGroup *group=dynamic_cast<QParallelAnimationGroup*>(sender());
    for(int i=0;i<group->animationCount();i++) {
        QPropertyAnimation *animation=dynamic_cast<QPropertyAnimation*>(group->animationAt(i));
        BikeItem *bike=dynamic_cast<BikeItem*>(animation->targetObject());
        if (bike) {
            bike->hide();
            setFreeBike(bike);
        }
        PersonItem *person=dynamic_cast<PersonItem*>(animation->targetObject());
        if (person) {
